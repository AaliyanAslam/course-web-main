    ...
    <div className="block">
      ...
      <div
        ...
        className="
          ...
          z-50 animate-wiggle-offer
        "
      >
        <img
          src="/images/clock.gif"
          className="h-5 w-5 sm:h-6 sm:w-6 mix-blend-multiply"
          alt="Clock"
        />
        <div className="flex flex-col leading-tight">
          <span className="text-[10px] sm:text-xs opacity-90">Offer ends in</span>
          <span className="text-xs sm:text-sm font-bold tracking-wide">
            {formatTime(timeLeft)}
          </span>
        </div>

        <div className="hidden sm:flex bg-gradient-to-r from-yellow-100 to-teal-50 text-teal-700 text-xs font-semibold px-3 py-1.5 rounded-full shadow-sm items-center gap-1 ml-2 border border-teal-200">
          <span className="bg-teal-600 text-white text-[10px] px-2 py-1 rounded-full font-bold">
            NEW
          </span>
          <span className="flex items-center gap-1">
            <Gift size={16} /> Limited Offer
          </span>
        </div>
      </div>
      ...
      {/* 🧩 Modal */}
      {isOpen && (
        <div className="fixed inset-0 bg-black/30 backdrop-blur-xl flex items-center justify-center z-50 px-4">
          <div className="bg-white rounded-xl shadow-lg max-w-sm w-full p-6 sm:p-8 relative">
            {/* Close */}
            <button
            ...
            >
              <span className="text-gray-400 text-sm line-through block">$1779</span>
              <span className="text-xl sm:text-2xl font-bold text-teal-600 block">
                $1499
              </span>
            ...
            {/* Countdown */}
            <div
              className="mb-4 text-sm font-semibold text-red-600 text-center"
              style={{ fontFamily: "montserrat" }}
            >
            ...
            {/* Features */}
            <ul
              className="text-left text-gray-600 text-sm space-y-2"
              style={{ fontFamily: "montserrat" }}
            >
